...
pub trait CacheBackend: Send + Sync {
    ...
    async fn get_exchange_rate(&self, key: &str) -> Result<CachedResponse<ExchangeRateResponse>, CacheError>;
    
    /// Set a cached response for ExchangeRateResponse
    async fn set_exchange_rate(&self, key: &str, response: CachedResponse<ExchangeRateResponse>) -> Result<(), CacheError>;
    
    /// Get a cached response as raw JSON string
    async fn get_raw(&self, key: &str) -> Result<(String, DateTime<Utc>, DateTime<Utc>), CacheError>;
    
    /// Set a cached response as raw JSON string
    async fn set_raw(&self, key: &str, json: String, cached_at: DateTime<Utc>, expires_at: DateTime<Utc>) -> Result<(), CacheError>;
...
impl CacheBackend for InMemoryCache {
    async fn get_exchange_rate(&self, key: &str) -> Result<CachedResponse<ExchangeRateResponse>, CacheError> {
        ...
        let cache = self.cache.read().map_err(|e| CacheError::Backend(e.to_string()))?;
        let expiry = self.expiry.read().map_err(|e| CacheError::Backend(e.to_string()))?;
        ...
        let cached_response: CachedResponse<ExchangeRateResponse> = serde_json::from_str(json)
    ...
    async fn set_exchange_rate(&self, key: &str, response: CachedResponse<ExchangeRateResponse>) -> Result<(), CacheError> {
        ...
        let mut cache = self.cache.write().map_err(|e| CacheError::Backend(e.to_string()))?;
        let mut expiry = self.expiry.write().map_err(|e| CacheError::Backend(e.to_string()))?;
    ...
    async fn set_raw(&self, key: &str, json: String, _cached_at: DateTime<Utc>, expires_at: DateTime<Utc>) -> Result<(), CacheError> {
        // Store the serialized response and expiration time
        let mut cache = self.cache.write().map_err(|e| CacheError::Backend(e.to_string()))?;
        let mut expiry = self.expiry.write().map_err(|e| CacheError::Backend(e.to_string()))?;
...
pub mod sqlite {
    ...
    impl SqliteCache {
        ...
        pub fn new(db_path: impl AsRef<Path>) -> Result<Self, CacheError> {
            let conn = Connection::open(db_path)
                .map_err(|e| CacheError::Backend(format!("Failed to open SQLite database: {}", e)))?;
    ...
    impl CacheBackend for SqliteCache {
        async fn get_exchange_rate(&self, key: &str) -> Result<CachedResponse<ExchangeRateResponse>, CacheError> {
            let conn = self.conn.lock().map_err(|e| CacheError::Backend(e.to_string()))?;
            
            let mut stmt = conn
                .prepare("SELECT response, cached_at, expires_at FROM exchange_rate_cache WHERE key = ?")
